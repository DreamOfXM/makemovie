...
A catalog entry is a promise that can be checked, because probing materialises every model a catalog lists and probes it under that connection. So a catalog lists only what its adapter can actually drive: Seedance and Kling list text-to-video alone — Ark's because the adapter implements that one endpoint and no other, Kling's because image-to-video has never been exercised against a real account and a model name this repository cannot verify does not belong in a public catalog. Both reasons are recorded in the entry's `spec` rather than left to the issue tracker. A provider that is a protocol rather than a vendor ships **no** model list and no default host, because naming either one for an operator's private gateway would be a guess dressed up as a recommendation.
...
**`prompt_extend` stays off even though it buys quality.** The vendor's prompt rewriting would make the stored `requestSnapshot` describe a request nobody sent, and that snapshot is the artifact's whole traceability story — the reason a delivered clip can be explained, audited, and reproduced from the same words. A clip nobody can re-run is worth less than a slightly better clip, so the request pins `prompt_extend: false` in `videoParameters` and leaves the rewriting to whoever edits the storyboard text, where the change is versioned. `watermark: false` sits beside it for a different reason: it is already the vendor's default, and is written out only so a delivered master's freedom from a watermark is stated in the request rather than inherited from a doc page.
