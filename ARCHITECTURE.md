...
> Status: the platform layer (tenancy, RBAC, sessions, audit, and the model capability configuration center), the generation pipeline (stage batches, the BullMQ queue, worker execution with candidate fallback, the quality gate, artifact storage and streaming, and FFmpeg composition), source and script versioning with approval gating, episode asset authoring with reference-image generation, versioning, approval, and binding to storyboard shots, and acceptance-gated delivery manifests are implemented and covered by tests. The pipeline is AI-driven and self-advancing: the `SCRIPT` and `STORYBOARD` stages write real generated content back into the domain — a script from the approved source, storyboard shots from the approved script — and a completed batch auto-advances to the next eligible stage (script → storyboard → asset → first frames → video), which a human can also push one step at a time with a one-click **Advance pipeline**. Every hop is gated on the approval it needs, every stage's output stays editable, and re-running a stage after an edit creates a new revision rather than overwriting the old one. It is a human-on-the-loop pipeline: the chain runs itself up to the video stage and pauses for a person at each approval, while composition stays a deliberate click. Quality control has two checkers: a deterministic hash placeholder that is still the default, and a model-driven visual audit of images and single video frames that is implemented but has never been run against a live provider. The remaining work — chaining composition onto the end of the auto-advance, cascading a regenerate downstream automatically, AI asset extraction from the script, and the deep content audits — is design; section-level status is called out inline.
...
- Episode assets: authoring with a kind, name, and description; reference-image generation through the image slot; versioning; approval; and binding an asset to the storyboard shots that use it
...
- Orchestration boundaries still open: composition is not chained onto the auto-advance (it stays a deliberate human click), a regenerate does not yet cascade to downstream stages on its own, and assets are still authored by hand rather than extracted from the script by AI
...
- `apps/worker`: BullMQ consumer running generation tasks, auto-advancing the pipeline when a batch completes, and composing episodes
...
- `packages/pipeline`: generation orchestration — stage gates, prompt building, batching, auto-advance, and regenerate — shared by the API trigger and the worker relay so both paths plan identically
...
2. `IMAGE` and `VIDEO` plan one task per storyboard (optionally restricted by `storyboardIds`, which must all belong to the episode); `ASSET` plans one task per asset; `SCRIPT` and `STORYBOARD` plan a single episode-level task. The prompt is the storyboard title and description, the asset's `kind name: description`, or — for the content stages — the approved upstream text: `SCRIPT` is refused (`409`) without an approved source version and is prompted with it, and `STORYBOARD` is refused without an approved script version and is prompted to return a JSON shot list.
...
- On success the worker writes a `UsageLedger` entry (input prompt length, output byte count) and then writes generated content back into the domain before stamping the task `SUCCEEDED` with the winning provider, model, and response snapshot. An `ASSET`-stage task appends the next `AssetVersion` for the asset named in the request snapshot, pointing at the stored artifact and left `DRAFT` — a passed quality check is not a human approval of the likeness. A `SCRIPT`-stage task becomes a new `ScriptVersion` (deduplicated by checksum), and a `STORYBOARD`-stage task fans its JSON shot list out into `Storyboard` rows linked to the approved script. Writing content before `SUCCEEDED` means an unparseable result falls through to the next candidate instead of leaving a succeeded task with nothing to show.
...
Orchestration (`packages/pipeline`, shared by the API and the worker): the pipeline advances itself, but only as far as a human has approved. `PIPELINE_STAGES` is the auto-advance order — `SCRIPT → STORYBOARD → ASSET → IMAGE → VIDEO`; `AUDIO` and composition are deliberately not in it (voice/music is triggerable by hand, and composition stays a deliberate click). `nextRunnableStage` walks that order and returns the first stage that has not run yet (no batch exists for its database stage value — `IMAGE` runs as `FIRST_FRAME`, so the check goes through `stageDbValues`) and whose prerequisites are met: `SCRIPT` needs an approved source, `STORYBOARD` an approved script, `ASSET` at least one authored asset, and `IMAGE`/`VIDEO` an approved script and at least one storyboard. Two callers drive it:

- `POST /episodes/:episodeId/run-pipeline` (permission `generation:trigger`) advances one step on demand — the console's one-click **Advance pipeline**. It returns the stage it started and its batch, or `409 pipeline:nothingRunnable` when every stage is run or paused on a missing approval, which the console reports as "up to date" rather than as an error.
...
- Triggering `SCRIPT` with no approved source, or `STORYBOARD`/`IMAGE`/`VIDEO`/`AUDIO` with no approved script, is rejected with `409` and queues nothing
- A succeeded `SCRIPT` task writes a new draft `ScriptVersion` deduplicated by checksum, and a succeeded `STORYBOARD` task fans its JSON shot list out into `Storyboard` rows linked to the approved script; output that cannot be parsed fails the task rather than writing rows
...
- `POST /episodes/:id/run-pipeline` advances exactly one runnable stage, skipping a stage that already has a batch, and answers `409 pipeline:nothingRunnable` when the episode is up to date or paused on a missing approval
...
- Video generation has run against a live provider; only Qwen-Image image generation is proven live
- The auto-advance chain reaches composition: today it stops at `VIDEO`, and composition and `AUDIO` stay human-triggered
- A regenerate cascades: today nothing flags downstream stages stale when an upstream is edited, so each stage is re-run deliberately one at a time
- Characters, props, and scenes are extracted from the script by AI rather than hand-authored
- A generated `ScriptVersion` or `Storyboard` traces back to the task and artifact that produced it — the link is implicit today and needs a schema change to persist
- Storyboards are versioned: a shot can be transitioned to approved, but an edit rewrites the row in place, so there is no per-revision history the way sources and scripts have — and because nothing supersedes a shot list, regenerating `STORYBOARD` leaves the new breakdown alongside the old one, with no endpoint to delete a shot that should not be there
