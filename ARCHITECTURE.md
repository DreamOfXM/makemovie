...
What limits an operator is not how many vendors exist but how many slots the chain resolves, so the Model Center opens on readiness rather than on a vendor list. The page states the arithmetic it got from the code — a score over the **four required slots**, and the one catalog that covers all four on its own — and the wider claim sits behind it: **seven slots are consumed anywhere in the product, and a single DashScope connection can serve all seven.**

The numbers come from `stageSlots` in `packages/pipeline`, not from the domain enum. `SCRIPT → script_text`, `STORYBOARD → storyboard_text`, `ASSET` and `IMAGE → image_gen`, `VIDEO → video_t2v`, `AUDIO → tts_voice`, `MUSIC → music_gen`, plus `visual_audit` for the worker's model-driven quality check: seven consumed slots, of which the first four gate the finished cut. `video_i2v` and `video_r2v` exist in `CapabilitySlot` because the modality is real and some vendors sell it, but **no pipeline stage resolves them** — so the readiness view lists them on a separate line as "not yet wired into the chain" instead of counting them as gaps. Asking someone to buy a model the code never calls is the same failure as a control that scrolls nowhere, and the UI rule that forbids the second forbids the first.

Each row also says what absence costs, because "missing" means four different things:
...
A catalog entry is a promise that can be checked, because probing materialises every model a catalog lists and probes it under that connection. So a catalog lists only what its adapter can actually drive and what a production stage can actually bind: Seedance and Kling list text-to-video alone — Ark's image-to-video wants a first frame at a *public* URL, which contradicts keeping frames in the product's own storage, and Kling's wants base64 the adapter does build but no stage can bind yet. Both reasons are recorded in the entry's `spec` rather than left to the issue tracker. A provider that is a protocol rather than a vendor ships **no** model list and no default host, because naming either one for an operator's private gateway would be a guess dressed up as a recommendation.
